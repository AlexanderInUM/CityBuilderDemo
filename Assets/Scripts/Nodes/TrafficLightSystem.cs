	...
	void Update () {
		...
		if (time > greenLightTime + yellowLightTime + redLightTime) {
			...
			if (lastLight) {
				...
				gameObject.GetComponent<UnityEngine.AI.NavMeshObstacle> ().size = new Vector3 (15.0f,2.0f,21.0f);
			} else {
				horzLights[0].GetComponent<Renderer>().material = greenLight;
				horzLights[1].GetComponent<Renderer>().material = greenLight;
				...
				gameObject.GetComponent<UnityEngine.AI.NavMeshObstacle> ().size = new Vector3 (21.0f,2.0f,15.0f);
			}
		}
